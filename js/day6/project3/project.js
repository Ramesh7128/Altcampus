...
function uniqueArray(words, checkWord) {
    return words.includes(checkWord)
};
...
function sumPopulation(data) {
    let countrySum = data.reduce((acc, country) => {
  ...
  function summarize(fruitBasket) {
      let sumObject = fruitBasket.reduce((acc, fruit) => {
          if (fruit in acc)  {
