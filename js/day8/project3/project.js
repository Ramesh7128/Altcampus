...
function randomQoutes() {
    return quotes[Math.floor(Math.random() * Math.floor(quotes.length-1))];
}
...
document.addEventListener('keydown', function(event) {
    console.log(event);
    if (event.keyCode == 32) {
        var newquoteObject = randomQoutes();
        document.getElementById('changeQuote').innerHTML = `<p>${newquoteObject.quoteText}</p><p>-<i>${newquoteObject.quoteAuthor}<i></p>`;
    }
});
